...
const main = async () => {
    ...
    const amountOut = ethers.parseUnits("10000", 6); // Amount Desired for address A
  const amountDAIMin = ethers.parseUnits("9000", 18); // Amount expected after slippage tolerance (Stop loss)
    ...
    const tx = await ROUTER.swapTokensForExactTokens( amountUSDC, amountDAIMin, path, impersonatedSigner.address, deadline);
